...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC1': PC1_LT, 'PC2': PC2_LT, 'PC3': PC3_LT, 'PC5': PC5_LT, 'PC7': PC7_LT, 'PC8': PC8_LT, 'PC10': PC10_LT, 'PC13': PC13_LT, 'PC14': PC14_LT, 'PC15': PC15_LT, 'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC1 + PC2 + PC3 + PC5 + PC7 + PC8 + PC10 + PC13 + PC14 + PC15", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
OLS_PC_15LT = Model_results_PC_LTcol[10] # Coefficient PC15
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT) + (OLS_PC_3LT*PC3_LT) + (OLS_PC_5LT*PC5_LT) + (OLS_PC_7LT*PC7_LT) + (OLS_PC_8LT*PC8_LT) + (OLS_PC_10LT*PC10_LT) + (OLS_PC_13LT*PC13_LT) + (OLS_PC_14LT*PC14_LT) + (OLS_PC_15LT*PC15_LT))
