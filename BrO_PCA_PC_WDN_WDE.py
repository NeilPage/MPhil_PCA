...
#------------------------------------------------------------------------------
# SPLIT WIND DIRECTION (DEGREES) INTO NORTHINGS & EASTINGS

#---------------
# Northings = WindSpeed * cos(WindDir) # WD needs to be converted from degrees to radians
# Eastings  = WindSpeed * sin(WindDir) # WD needs to be converted from degrees to radians
#---------------

dfPCA['MD_WD'] = dfPCA['WindDir']-270
Filter = dfPCA['MD_WD'] <=0

if dfPCA['MD_WD'] <=0:
    Test = dfPCA['MD_WD'] + 360
else:
    Test = dfPCA['MD_WD']

dfPCA['WD_Northing'] = dfPCA['WS10m'] * np.cos(np.radians(dfPCA['WindDir']))
dfPCA['WD_Easting']  = dfPCA['WS10m'] * np.sin(np.radians(dfPCA['WindDir']))
...
# Plot WD
ax.plot(dfPCA['WindDir'], dfPCA['WindDir']*-0.322, marker='o', c='g', markersize = 2.0, linestyle='none')

ax.plot(dfPCA.index, dfPCA['WindDir']*-0.322, marker='o', c='r', markersize = 2.0, linestyle='none')
ax2.plot(dfPCA.index, dfPCA['WindDir'], marker='o', c='g', markersize = 2.0, linestyle='none')

ax.plot(dfPCA.index, dfPCA['WD_Northing'], marker='o', c='r', markersize = 2.0, linestyle='none')
ax.plot(dfPCA.index, dfPCA['WD_Easting'], marker='o', c='b', markersize = 2.0, linestyle='none')
#ax.plot(dfPCA['WD_Easting'], dfPCA['WD_Northing'], marker='o', c='r', markersize = 2.0, linestyle='none')
...
AdditionalVariables = dfPCA_Standard3.drop(['SurfBrO','LTBrO','sqrt_SurfBrO','sqrt_LTBrO','AEC','WindDir','IceContactPerc','LandContactMLH','Weighted_Ice','Weighted_Land','Weighted_Ocean',
                                            'Percentage_Ice','Percentage_Land','Percentage_Ocean','InfraRed','Fluro','nd100m','SurfBrO2','SurfBrO3','sqrt_SurfBrO2','sqrt_SurfBrO3'], 1)

# BrO_Surf Varaiables
BrOSurfVariables    = dfPCA_Standard3.drop(['SurfBrO','LTBrO','sqrt_SurfBrO','sqrt_LTBrO','AEC','WindDir','IceContact100m','IceContactMLH','IceContactPerc',
                                            ...
                                            'Chlorophyll','SurfBrO2','SurfBrO3','sqrt_SurfBrO2','sqrt_SurfBrO3','WD_Northing','WD_Easting'], 1)

# BrO_LTcol Varaiables
BrOLTcolVariables   = dfPCA_Standard3.drop(['SurfBrO','LTBrO','sqrt_SurfBrO','sqrt_LTBrO','AEC','WindDir','IceContact100m','IceContactMLH','IceContactPerc',
                                            'LandContact','LandContactMLH','OceanContact','Weighted_Ice','Weighted_Land','Weighted_Ocean','Percentage_Ice',
...
df_PC_BrOLTcol = pd.concat([PC1_LT,PC2_LT,PC3_LT,PC4_LT,PC5_LT,PC6_LT,PC7_LT,PC8_LT,PC9_LT,PC10_LT,PC11_LT,PC12_LT,PC13_LT,PC14_LT,PC15_LT,PC16_LT], axis=1, join='inner')
df_PC_BrOLTcol.columns = ['PC1','PC2','PC3','PC4','PC5','PC6','PC7','PC8','PC9','PC10','PC11','PC12','PC13','PC14','PC15','PC16']
...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC1': PC1_LT, 'PC2': PC2_LT, 'PC3': PC3_LT, 'PC4': PC4_LT, 'PC5': PC5_LT, 'PC6': PC6_LT, 'PC7': PC7_LT, 'PC8': PC8_LT, 'PC9': PC9_LT, 'PC10': PC10_LT, 'PC11': PC11_LT, 'PC12': PC12_LT, 'PC13': PC13_LT, 'PC14': PC14_LT, 'PC15': PC15_LT, 'PC16': PC16_LT,'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT) + (OLS_PC_3LT*PC3_LT) + (OLS_PC_4LT*PC4_LT) + (OLS_PC_5LT*PC5_LT) + (OLS_PC_6LT*PC6_LT) + (OLS_PC_7LT*PC7_LT) + (OLS_PC_8LT*PC8_LT) + (OLS_PC_9LT*PC9_LT) + (OLS_PC_10LT*PC10_LT) + (OLS_PC_11LT*PC11_LT) + (OLS_PC_12LT*PC12_LT) + (OLS_PC_13LT*PC13_LT) + (OLS_PC_14LT*PC14_LT) + (OLS_PC_15LT*PC15_LT) + (OLS_PC_16LT*PC16_LT))
#Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT))
...
slope_WD_North_LT,       intercept_WD_North_LT,       r_WD_North_LT,       p_WD_North_LT,       std_err_WD_North_LT       = stats.linregress(dataLT['z'], dataLT['WD_Northing'])
slope_WD_East_LT,        intercept_WD_East_LT,        r_WD_East_LT,        p_WD_East_LT,        std_err_WD_East_LT        = stats.linregress(dataLT['z'], dataLT['WD_Easting'])
...
r2_WD_North_LT       = r_WD_North_LT**2
r2_WD_East_LT        = r_WD_East_LT**2
...
# BrO LTcol
#--------------
dfLinearRegressionLT = pd.DataFrame({'R (BrO_LTcol)':     [r_O3_LT,  r_AEC_LT,  r_SurfTemp_LT,  r_SLP_LT,  r_WS10m_LT,  r_MLH_LT,  r_PTD1000m_LT,  r_PTD100m_LT,  r_WD_North_LT,  r_WD_East_LT,  r_SolRad_LT,  r_IceContact_LT,  r_SeaIceConc_LT,  r_Chloro_LT,  r_WaterTemp_LT,  r_RelHum_LT],
                                   'R^2 (BrO_LTcol)':     [r2_O3_LT, r2_AEC_LT, r2_SurfTemp_LT, r2_SLP_LT, r2_WS10m_LT, r2_MLH_LT, r2_PTD1000m_LT, r2_PTD100m_LT, r2_WD_North_LT, r2_WD_East_LT, r2_SolRad_LT, r2_IceContact_LT, r2_SeaIceConc_LT, r2_Chloro_LT, r2_WaterTemp_LT, r2_RelHum_LT],
                                   'p-value (BrO_LTcol)': [p_O3_LT,  p_AEC_LT,  p_SurfTemp_LT,  p_SLP_LT,  p_WS10m_LT,  p_MLH_LT,  p_PTD1000m_LT,  p_PTD100m_LT,  p_WD_North_LT,  p_WD_East_LT,  p_SolRad_LT,  p_IceContact_LT,  p_SeaIceConc_LT,  p_Chloro_LT,  p_WaterTemp_LT,  p_RelHum_LT]})
dfLinearRegressionLT.index = ['O3','AEC','SurfTemp','SLP','WS10m','MLH','PTD1000m','PTD100m','WD_Northing','WD_Easting','SolRad','IceContact','SeaIceConc','Chlorophyll','WaterTemp','RelHum']
...
model_MLR_LTcol = ols("z ~ O3 + log_AEC + SurfTemp + SurfPres + WS10m + MLH + PTDif1000m + PTDif100m + WD_Northing + WD_Easting + SolRad + IceContact + SeaIceConc + Chlorophyll + Water_Temp + RelHum", dataLT).fit() # LTcol StatsModel (ols)
...
OLS_MLR_7LT  = Model_results_MLR_LTcol[7]  # Coefficient MLH
...
OLS_MLR_8LT  = Model_results_MLR_LTcol[8]  # Coefficient SolRad
OLS_MLR_9LT  = Model_results_MLR_LTcol[9] # Coefficient IceContact
OLS_MLR_10LT = Model_results_MLR_LTcol[10] # Coefficient SeaIceConc
#OLS_MLR_7LT  = Model_results_MLR_LTcol[13] # Coefficient WeightedLand
OLS_MLR_11LT = Model_results_MLR_LTcol[11] # Coefficient Chlorophyll
OLS_MLR_12LT = Model_results_MLR_LTcol[12] # Coefficient WaterTemp
#OLS_MLR_15LT = Model_results_MLR_LTcol[15] # Coefficient WaterSal
OLS_MLR_13LT = Model_results_MLR_LTcol[13] # Coefficient RelHum
...
OLS_MLR_14LT = Model_results_MLR_LTcol[14] # Coefficient log_AEC
OLS_MLR_15LT = Model_results_MLR_LTcol[15] # Coefficient WD_Nothing
OLS_MLR_16LT = Model_results_MLR_LTcol[16] # Coefficient WD_Easting
...
Pred_BrOLTcol_MLR = np.square(OLS_MLR_0LT + (OLS_MLR_1LT*BrOLTcolVariables['O3']) + (OLS_MLR_2LT*BrOLTcolVariables['SurfPres']) + (OLS_MLR_3LT*BrOLTcolVariables['SurfTemp']) + (OLS_MLR_4LT*BrOLTcolVariables['PTDif100m']) + (OLS_MLR_5LT*BrOLTcolVariables['PTDif1000m']) + (OLS_MLR_6LT*BrOLTcolVariables['WS10m'])    + (OLS_MLR_7LT*BrOLTcolVariables['MLH'])      + (OLS_MLR_8LT*BrOLTcolVariables['SolRad'])   + (OLS_MLR_9LT*BrOLTcolVariables['IceContact']) + (OLS_MLR_10LT*BrOLTcolVariables['SeaIceConc']) + (OLS_MLR_11LT*BrOLTcolVariables['Chlorophyll']) + (OLS_MLR_12LT*BrOLTcolVariables['Water_Temp']) + (OLS_MLR_13LT*BrOLTcolVariables['RelHum']) + (OLS_MLR_14LT*BrOLTcolVariables['log_AEC']) + (OLS_MLR_15LT*BrOLTcolVariables['WD_Northing']) + (OLS_MLR_16LT*BrOLTcolVariables['WD_Easting']))
...
dfPC_IntCoef = {'Intercept (B0)':       [OLS_PC_0S, OLS_PC_0LT],
                ...
                'Coefficient 8 (B8)':   [OLS_PC_8S, OLS_PC_8LT],
                'Coefficient 9 (B9)':   [OLS_PC_9S, OLS_PC_9LT],
                'Coefficient 10 (B10)': [OLS_PC_10S,OLS_PC_10LT],
                'Coefficient 11 (B11)': [np.nan,    OLS_PC_11LT],
                'Coefficient 12 (B12)': [np.nan,    OLS_PC_12LT],
                'Coefficient 13 (B13)': [np.nan,    OLS_PC_13LT],
                'Coefficient 14 (B14)': [np.nan,    OLS_PC_14LT],
                'Coefficient 15 (B15)': [np.nan,    OLS_PC_15LT],
                'Coefficient 16 (B16)': [np.nan,    OLS_PC_16LT]}
dfPC_IntCoef = pd.DataFrame(dfPC_IntCoef, index = ['ols_Surf','ols_LTcol'],columns = ['Intercept (B0)','Coefficient 1 (B1)','Coefficient 2 (B2)','Coefficient 3 (B3)','Coefficient 4 (B4)',
                                                                                      ...
                                                                                      'Coefficient 15 (B15)','Coefficient 16 (B16)'])
