...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC1': PC1_LT, 'PC2': PC2_LT, 'PC3': PC3_LT, 'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC1 + PC2 + PC3", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT) + (OLS_PC_3LT*PC3_LT))
