...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC2': PC2_LT, 'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC2", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
OLS_PC_2LT  = Model_results_PC_LTcol[1]  # Coefficient PC2
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_2LT*PC2_LT))
...
dfPC_IntCoef = {'Intercept (B0)':       [OLS_PC_0S, OLS_PC_0LT],
                'Coefficient 1 (B1)':   [OLS_PC_1S, np.nan],
