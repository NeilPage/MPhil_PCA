...
BrOLTcolVariables   = dfPCA_Standard3.drop(['SurfBrO','LTBrO','sqrt_SurfBrO','sqrt_LTBrO','AEC','IceContact100m','IceContactMLH','IceContactPerc',
                                            'LandContact','LandContactMLH','OceanContact','Weighted_Ice','Weighted_Land','Weighted_Ocean','Percentage_Ice',
                                            'Percentage_Land','Percentage_Ocean','P1hr','nd100m','InfraRed','Fluro','Water_Sal', 'WindDir',
...
Principal_Variables_DfLT = pd.DataFrame(data = PrincipalComponents_VariablesLT, columns = ['PC1','PC2','PC3','PC4','PC5','PC6','PC7','PC8','PC9',
                                                                                       'PC10','PC11','PC12','PC13','PC14'])#,'PC16','PC17',
                                                                                       #'PC18','PC19','PC20','PC21','PC22','PC23','PC24','PC25','PC26']) # Variables
...
df_PC_BrOLTcol = pd.concat([PC1_LT,PC2_LT,PC3_LT,PC4_LT,PC5_LT,PC6_LT,PC7_LT,PC8_LT,PC9_LT,PC10_LT,PC11_LT,PC12_LT,PC13_LT,PC14_LT], axis=1, join='inner')
df_PC_BrOLTcol.columns = ['PC1','PC2','PC3','PC4','PC5','PC6','PC7','PC8','PC9','PC10','PC11','PC12','PC13','PC14']
...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC1': PC1_LT, 'PC2': PC2_LT, 'PC3': PC3_LT, 'PC4': PC4_LT, 'PC5': PC5_LT, 'PC6': PC6_LT, 'PC7': PC7_LT, 'PC8': PC8_LT, 'PC9': PC9_LT, 'PC10': PC10_LT, 'PC11': PC11_LT, 'PC12': PC12_LT, 'PC13': PC13_LT, 'PC14': PC14_LT, 'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
OLS_PC_14LT = Model_results_PC_LTcol[14] # Coefficient PC14
#OLS_PC_15LT = Model_results_PC_LTcol[15] # Coefficient PC15
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT) + (OLS_PC_3LT*PC3_LT) + (OLS_PC_4LT*PC4_LT) + (OLS_PC_5LT*PC5_LT) + (OLS_PC_6LT*PC6_LT) + (OLS_PC_7LT*PC7_LT) + (OLS_PC_8LT*PC8_LT) + (OLS_PC_9LT*PC9_LT) + (OLS_PC_10LT*PC10_LT) + (OLS_PC_11LT*PC11_LT) + (OLS_PC_12LT*PC12_LT) + (OLS_PC_13LT*PC13_LT) + (OLS_PC_14LT*PC14_LT))
...
# BrO LTcol
#--------------
dfLinearRegressionLT = pd.DataFrame({'R (BrO_LTcol)':     [r_O3_LT,  r_AEC_LT,  r_SurfTemp_LT,  r_SLP_LT,  r_WS10m_LT,  r_MLH_LT,  r_PTD1000m_LT,  r_PTD100m_LT,  r_SolRad_LT,  r_IceContact_LT,  r_SeaIceConc_LT,  r_Chloro_LT,  r_WaterTemp_LT,  r_RelHum_LT],
                                   'R^2 (BrO_LTcol)':     [r2_O3_LT, r2_AEC_LT, r2_SurfTemp_LT, r2_SLP_LT, r2_WS10m_LT, r2_MLH_LT, r2_PTD1000m_LT, r2_PTD100m_LT, r2_SolRad_LT, r2_IceContact_LT, r2_SeaIceConc_LT, r2_Chloro_LT, r2_WaterTemp_LT, r2_RelHum_LT],
                                   'p-value (BrO_LTcol)': [p_O3_LT,  p_AEC_LT,  p_SurfTemp_LT,  p_SLP_LT,  p_WS10m_LT,  p_MLH_LT,  p_PTD1000m_LT,  p_PTD100m_LT,  p_SolRad_LT,  p_IceContact_LT,  p_SeaIceConc_LT,  p_Chloro_LT,  p_WaterTemp_LT,  p_RelHum_LT]})
dfLinearRegressionLT.index = ['O3','AEC','SurfTemp','SLP','WS10m','MLH','PTD1000m','PTD100m','SolRad','IceContact','SeaIceConc','Chlorophyll','WaterTemp','RelHum']
...
model_MLR_LTcol = ols("z ~ O3 + log_AEC + SurfTemp + SurfPres + WS10m + MLH + PTDif1000m + PTDif100m + SolRad + IceContact + SeaIceConc + Chlorophyll + Water_Temp + RelHum", dataLT).fit() # LTcol StatsModel (ols)
