...
# BrO LTcol
dataPCA_LT = pd.DataFrame({'PC1': PC1_LT, 'PC2': PC2_LT, 'PC3': PC3_LT, 'PC10': PC10_LT, 'PC14': PC14_LT, 'z': dfPCA['sqrt_LTBrO']})

# Fit the model
model_PC_Surf  = ols("z ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10", dataPCA_S).fit()  # Surface StatsModel (ols)
model_PC_LTcol = ols("z ~ PC1 + PC2 + PC3 + PC10 + PC14", dataPCA_LT).fit() # LTcol StatsModel (ols)
...
OLS_PC_10LT = Model_results_PC_LTcol[4] # Coefficient PC10
...
OLS_PC_14LT = Model_results_PC_LTcol[5] # Coefficient PC14
...
Pred_BrOLTcol_OLS = np.square(OLS_PC_0LT + (OLS_PC_1LT*PC1_LT) + (OLS_PC_2LT*PC2_LT) + (OLS_PC_3LT*PC3_LT) + (OLS_PC_10LT*PC10_LT) + (OLS_PC_14LT*PC14_LT))
